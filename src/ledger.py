...
ENTRIES: list[tuple] = [
    ...
    ("HH", "論文どおり", "総務省 住民基本台帳世帯数（日本人、全国、年度。e-Stat 社会・人口統計体系 0000010101 の A7103）", "data/raw/estat_hh_0000010101.csv",
     "年度値を年度末（翌年Q1）に置いて対数線形で四半期補間", "世帯 ÷1万 → 1万世帯", "直近年度末より先は直近1年の変化率で延長", None, "#3"),
    ...
    ("SBGV", "定義の解釈", "日本銀行 資金循環統計 一般政府 資産合計 − 金融資産・負債差額（ストック、FF/FOF_FFAS420A900, FOF_FFAS420L700）= 負債",
     ...
     "億円 ÷10 → 10億円", "なし", None, ""),
    ("OTNGV", "論文どおり", "一般政府財政バランス残余項目 = SNA 一般政府の純貸出(+)/純借入(-)（年度、GFS）− 式126 の各項目による計算値の年度平均（論文の出所 Author）",
     f"{RAWDIR}/{V['annual']}s6_2_jp.xlsx", "年度値を年度内一定で四半期に割付", "10億円・年率", "年度値のない端の期間は最後の年度の名目GDP比で延長", None, ""),
    ("BGV", "論文どおり", "式126（OTNGV を含む）。年度平均が ESRI 国民経済計算 一般政府の部門別勘定（GFS）の純貸出(+)/純借入(-) に一致",
     f"{RAWDIR}/{V['annual']}s6_2_jp.xlsx", "式126 を実績に当てはめ、OTNGV で SNA の年度値に合わせる", "10億円・年率", "なし", None, ""),
    ...
    ("PLAND", "代用", "LANDT を 2015年平均=1 に指数化。論文は日本不動産研究所 市街地価格指数（有償。日本統計年鑑の掲載は2014年3月末まで）。"
     "式119 には対数で入り、水準・傾きを変えても乗数は変わらない（誤差修正項を全部動かしても差0）", "", "土地残高の変化率を地価の変化率とみなす", "2015年=1", "なし", None, "#3"),
