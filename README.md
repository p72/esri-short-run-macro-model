...
| PR | 内容 | 論文の乗数表との一致率 |
|---|---|---|
| #4 | 出力 CSV を小数第8位で丸め、実行ごとの微小な揺れをなくす。統計上の不突合 `SDV` の注記を実データに合わせて修正 | 96.1% |
| #5 | 変数台帳（`src/ledger.py`）を追加。全変数の論文の定義・出所と、実際の系列・加工・単位換算・欠損処理を突き合わせて自動検査 | 96.1% |
| #7 | 忠実性の監査（`docs/fidelity.md`、係数・定数152本の機械照合）と、法人税減税シナリオの資本コスト `UCC` の寄与分解 | 96.1% |
| #14 | 財政バランスの残余項目 `OTNGV` を、SNA 一般政府の純貸出/純借入に合うよう逆算（論文の出所どおり）。財政収支/GDP の一致率が 93.9% → 100% | **96.2%** |
| #15 | 式129（所得実効税率）の左辺 `LOG` を、印刷された係数と決定係数から `DLOG` の誤植と判定。世帯数 `HH` を論文と同じ住民基本台帳世帯数に置き換え | 96.2% |

Issue #1〜#3 はすべて閉じた。

- **#1 資本コスト `UCC` の符号**: 残差として説明して完了。印刷どおりの式と公表統計で裏付けられる入力では、論文の符号は出ない。
- **#2 式129 の型**: 誤植と判定して完了。水準式なら決定係数は約0.9になるはずで、印刷の0.48は DLOG 型の回帰でしか出ない。
- **#3 代用系列**: 完了。対数で入る代用系列（`HH`・`PLAND`）は、水準や傾きを変えても乗数が変わらないことを確かめた。

### データの版と応用

| PR | 内容 |
|---|---|
| #6 | 実質GDP乗数の四半期経路図（論文 vs 再現、11シナリオ） |
| #8 | 2024年版のデータ（2010Q1〜2024Q4、2020年基準SNA）を追加。`ESRI_VINTAGE=2024` と `--start`/`--end` で、最近の経済を基準解にした乗数を計算できる |
| #9 | 名目GDP1%規模の消費税減税 vs 給付金の経路図（既定版） |
| #10 | 食料品の消費税 8%→1% vs 同額の給付金（2024年版）。消費だけに効く税率を追加し、物価と税収を別々に合わせる。減収は SNA の食料支出から約3.4兆円、または `--loss-tn 4.4` で報道の4.4兆円 |
| #11 | 同額の一律の消費税減税を追加 |
| #12 | 政府案どおりの2年間の時限措置（`--years 2`） |
...
| スクリプト | 確かめること |
|---|---|
| `src/check_transcription.py` | 152本の係数・定数が論文と一致するか |
| `src/experiment_fidelity.py` | 印刷どおり・別解釈・誤植修正前の式で解いた一致率 |
| `src/experiment_ecm_greedy.py` | 誤差修正項をどれだけ固定すると論文に合うか |
| `src/experiment_ucc.py` | 資本コストの寄与分解と感応度 |
| `src/experiment_itr_form.py` | 式129 の左辺が水準か DLOG か |
| `src/experiment_proxy_levels.py` | 代用系列の水準・傾きが乗数に影響しないか |
